...
class Code_thread(threading.Thread):
        ...
        def run(self):
            global DAC, WPt, WPpw, mode, qmode, qpid
...
class Parametros(tk.Frame):
    def set_K(self,n):
        ...
        K = n
    def CrearPID(self):
        global qpid, KpLabel, KiLabel, KdLabel
        pid = PID(float(KpLabel),float(KiLabel),float(KdLabel))
        qpid.set(pid)
            
    def __init__(self, parent, controller):
        global qpid, KpLabel, KiLabel, KdLabel,label2,label3,label4
        tk.Frame.__init__(self,parent)
        ...
        button5 = ttk.Button(self, text = "Confirmar PID",
                              command = lambda: [self.CrearPID()], image = "")
